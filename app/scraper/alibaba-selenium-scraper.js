...
export function alibabaScraper(userInput, maxPrice) {
        ...
        if (filterText.includes("new")) {
          ...
          try {
            ...
            try {
              ...
              console.log(
                `Standard click failed, trying JavaScriptExecutor...`,
              );
  ...
  const main = async () => {
    ...
    try {
      ...
      await driver.get(
        `https://www.alibaba.com/trade/search?fsb=y&mergeResult=true&ta=y&tab=all&searchText=${searchQuery}&pricet=${maxPrice}`,
      );
      ...
      // Scrape pages
      while (true) {
        const success = await scrapePage(
          driver,
          productDataList,
          index,
          filtersApplied,
      ...
      // Save scraped data
      const outputFile = "app/scraper/alibaba_results.json";
      await fs.writeFile(
        outputFile,
...
//alibabaScraper("man's t-shirt", 100);
